...
int main()
{
    ...
    for(auto & crd: card_nums)
    {
        for(auto & rng: ranges)
        {
            if(rng.is_in_rnage(crd))
            {
...
Range::Range(const std::string & line, std::ofstream & lg_file)
{
    ...
    range_min = line.substr(1, first_sep - 1);
...
bool Range::is_in_range(const std::string & test_str)
{
    ...
    for(int letter = 0; letter < range_min.size(); letter++)
    {
        ...
        {
            log_file <<"Digit: '" <<test_str[letter] <<"' is NOT in range";
            return false;
        }
        log_file <<"Digit: '" <<test_str[letter] <<"' is in range";
    }
